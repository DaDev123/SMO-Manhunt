...
void InfectionIcon::exeWait() {
    ...
    // Set role text based on whether player is seeker (infected) or hider (runner)
    if (mInfo->mIsPlayerIt) {
        al::setPaneStringFormat(this, "TxtRole", "Infected");
    } else {
        al::setPaneStringFormat(this, "TxtRole", "Runner");
    }
