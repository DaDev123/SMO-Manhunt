        ...
        if (mInvulnTime < 5) {
            ...
            if (PlayerFunction::isPlayerDeadStatus(playerBase)) {
                ...
                for (size_t i = 0; i < (size_t)mPuppetHolder->getSize(); i++) {
                    ...
                    if (!other->isConnected || !other->isInSameStage || other->infectionIsRunning() || isYukimaru) {
                        continue;
                    }
...
bool InfectionMode::showNameTag(PuppetInfo* other) {
    return (
        ...
        || (isPlayerInfected() && other->infectionIsInfected())
        || (!isPlayerInfected() && !other->infectionIsInfected()) // Show names when both are hiders
    );
}
